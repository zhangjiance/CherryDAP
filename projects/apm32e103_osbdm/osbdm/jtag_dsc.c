...
int32_t t_write_ad(uint32_t addr, uint8_t *data) { return 0; }
int32_t t_read_ad(uint32_t addr, uint8_t *data) { return 0; }
...
Once_Send_Receive(char bitcount, char outval){
	...
	for(b=0; b<bitcount; b++){
		if(b == lastbit)
            inbit = TCLK_transition(TMS_HIGH_VAL, (outval & 0x01));		// Shift transition  (Exit1)
		else
            inbit = TCLK_transition(TMS_LOW_VAL,  (outval & 0x01));		// Shift transition
...
ScanIO(char ir, char bits, unsigned long val){
	...
	TCLK_transition(TMS_HIGH_VAL,0);	// Select Data Scan

	if(ir == INSTRUCTION_REGISTER){
		TCLK_transition(TMS_HIGH_VAL,0);	// Select Instr Scan
	}
	TCLK_transition(TMS_LOW_VAL,0);		// Capture
	TCLK_transition(TMS_LOW_VAL,0);		// Shift

	ival = Once_Send_Receive(bits, val);	  // Debug Request instruction
	TCLK_transition(TMS_HIGH_VAL,0);	// Update
	TCLK_transition(TMS_LOW_VAL,0);		// Run-Test/Idle
	return ival;
}
...
dsc_startup (int nst)
{
  ...
  output_buffer_enable_value = 0x80; // EMBEDDED TOWER
	...
	for (i=0; i<10; i++) TCLK_transition(TMS_HIGH_VAL,0);		// Transition CLK line: low_high
